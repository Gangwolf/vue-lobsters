<template>
  ...
  <div v-if="loaded">
    ...
    <div class="col-sm-6 col-sm-offset-2 buttons">
      <button v-show="showBackButton" @click="turnPage('hottest', 'backward')">Back</button>
      <button @click="turnPage('hottest', 'forward')">Next</button>
    </div>
  </div>
</template>
...
export default {
  ...
  ready() {
    this.getJSON('hottest', pageNumber);
  }
};
</script>